...
import HelpBox from '../../components/operate/HelpBox'
import MapControl from '../../components/operate/MapControl'
import MapControlToolbar from '../../components/operate/MapControlToolbar'
import CityListPanel from '../../components/operate/CityListPanel'
import * as mapActions from '../../actions/operate/MapAction'
...
class OperateBodyContainer extends Component {
    ...
    menuToggleAction(key) {
        ...
        let helpStyle = this.refs.operateHelpContainer.style
        ...
        switch (key) {
            ...
            case 'help':
                if (helpStyle.display == 'block') {
                    helpStyle.display = 'none'
                } else if (helpStyle.display == 'none') {
                    helpStyle.display = 'block'
                }
                return helpStyle.display
            default:
                return 'none'
        }
    }
    ...
    render() {
        ...
        let operateDisplayInterface = <div className="page-body-container" style={bodyStyle}>
            <div className="operate-body">
                ...
                <div className="operate-help-container" style={{display:'none'}} ref="operateHelpContainer">
                    <HelpBox/>
                </div>
                <div className="operate-map-container" style={{display:'block'}} ref="operateMapContainer">
                    <MapBox solution={operate.solution} actions={operateActions} map={map}
                        {...mapActions}/>
                    <MapControl solution={operate.solution} actions={operateActions} map={map} {...mapActions}/>
                    <MapControlToolbar solution={operate.solution} actions={operateActions} map={map} {...mapActions}/>

                    <CityListPanel solution={operate.solution} actions={operateActions} map={map}{...mapActions}/>
                </div>
            </div>
        </div>
