...
/**
 * a Task that reads mutations from a mongodb for storage in Kafka.
 */
public class MongodbSourceTask extends SourceTask {
    private final static Logger log = Logger.getLogger(MongodbSourceTask.class);
    ...
    public List<SourceRecord> poll() throws InterruptException {
        ...
        while (!reader.messages.isEmpty() && records.size() < batchSize) {
            ...
            records.add(new SourceRecord(Collections.singletonMap("mongodb", db),
