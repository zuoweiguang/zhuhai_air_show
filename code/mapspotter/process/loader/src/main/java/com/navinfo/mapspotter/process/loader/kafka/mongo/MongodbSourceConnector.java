...
/**
 * implements the connector interface to write on Kafka mutations
 * received from a mongodb database
 */
public class MongodbSourceConnector extends SourceConnector {
    private final static Logger log = Logger.getLogger(MongodbSourceConnector.class);
    ...
    public List<Map<String, String>> taskConfigs(int maxTasks) {
        ...
        for (int i = 0; i < maxTasks; i++) {
            ...
            config.put(DATABASES, StringUtil.join(dbsGrouped.get(i), ","));
            configs.add(config);
        }
