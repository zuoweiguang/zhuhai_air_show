...
/**
 * implement the Connector interface to send Kafka data to Mongodb
 */
public class MongodbSinkConnector extends SinkConnector {
    private final static Logger log = Logger.getLogger(MongodbSinkConnector.class);
    ...
    public void start(Map<String, String> map) {
        ...
        collections = Arrays.asList(map.get(COLLECTIONS).split(","));
        topics = Arrays.asList(map.get(TOPICS).split(","));

        if (collections.size() != topics.size()) {
    ...
    public List<Map<String, String>> taskConfigs(int maxTasks) {
        ...
        int numGroups = Math.min(collections.size(), maxTasks);

        List<List<String>> dbsGrouped = ConnectorUtils.groupPartitions(collections, numGroups);
        ...
        for (int i = 0; i < maxTasks; i++) {
            ...
            config.put(COLLECTIONS, StringUtil.join(dbsGrouped.get(i), ","));
            config.put(TOPICS, StringUtil.join(topicsGrouped.get(i), ","));
            configs.add(config);
        }
