 ...
 */
public class BasicRoadMapper extends Mapper<LongWritable, Text, Text, Text> {
    ...
    protected void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
        ...
        for (Map.Entry<String, StringBuilder> entry : builderMap.entrySet()) {
            ...
            valText.set(entry.getValue().toString());
    ...
    public void gridOperate(int xStart, int yStart, int xEnd, int yEnd, int laneNum, String pidstr, Map<String, StringBuilder> builderMap) throws IOException, InterruptedException {
        /**
         * 特殊处理由于接下来斜率需要用tDeltaX做分母所以需要特殊处理
         */
        if (xEnd == xStart) {
            if (yStart > yEnd) {
                int tmp = yStart;
        ...
        /**
         * 斜率在[-1,1]之间 纵向buffer，否则横向buffer
         */
        if (k > 1 || k < -1) {
            if (yStart > yEnd) {
                int tmp = yStart;
    ...
    /**
     * 拼接字符串为 x,y,pid_len|x,y,pid_len|x,y,pid_len|
     * @param mCode
     * @param builderMap
     * @param coordinate
     * @param pid
