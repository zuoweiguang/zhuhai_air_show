...
export default class BrandAnimationRender {
    ...
    init() {
        ...
        if(!map.getSource("animationSource_tmcBrand")){
            map.addSource("animationSource_tmcBrand", {
                "type": "geojson",
                "data": {
        ...
        for (let j = 0; j < typeCodes.length; j++) {
            ...
            let image = j + 1
            this.map.addLayer({
                "id": "animationLayer_tmcBrand" + j,
                "type": "symbol",
                "source": "animationSource_tmcBrand",
        ...
        const startAnimation = ()=> {
            if (zoom >= 8 && zoom <= 17) {
                env.brandAnimation.timeoutSet.add(setTimeout(()=> {
                    ...
                    map.getSource('animationSource_tmcBrand').setData(_tracks.data)
                }, 200))
            }
        }
